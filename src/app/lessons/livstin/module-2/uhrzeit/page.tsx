...
import { ArrowLeft, BookOpen, Play, Clock, Zap, CheckCircle, ArrowRight, Clock3 } from 'lucide-react'
...
const slides: Slide[] = [
      ...
      sections: [
        {
          subtitle: 'Часы',
          examples: [
            { number: '1', word: 'ein Uhr', sound: 'айн ур' },
            { number: '2', word: 'zwei Uhr', sound: 'цвай ур' },
            { number: '3', word: 'drei Uhr', sound: 'драй ур' },
            { number: '4', word: 'vier Uhr', sound: 'фир ур' },
            { number: '5', word: 'fünf Uhr', sound: 'фюнф ур' },
            { number: '6', word: 'sechs Uhr', sound: 'зэкс ур' },
            { number: '7', word: 'sieben Uhr', sound: 'зибэн ур' },
            { number: '8', word: 'acht Uhr', sound: 'ахт ур' },
            { number: '9', word: 'neun Uhr', sound: 'нойн ур' },
            { number: '10', word: 'zehn Uhr', sound: 'цэн ур' },
            { number: '12', word: 'zwölf Uhr', sound: 'цвёльф ур' },
            { number: '24', word: 'vierundzwanzig Uhr', sound: 'фир-унд-цванциг ур' }
          ]
        },
        {
          subtitle: 'Половина и четверть',
          examples: [
            { number: 'half', word: 'halb', sound: 'хальб' },
            { number: 'quarter', word: 'Viertel', sound: 'фиртель' },
            { number: 'past', word: 'nach', sound: 'нах' },
            { number: 'to', word: 'vor', sound: 'фор' }
          ]
        },
        {
          subtitle: 'Примеры',
          examples: [
            { number: '3:30', word: 'halb vier', sound: 'хальб фир' },
            { number: '3:15', word: 'Viertel nach drei', sound: 'фиртель нах драй' },
            { number: '3:45', word: 'Viertel vor vier', sound: 'фиртель фор фир' },
            { number: '3:05', word: 'fünf nach drei', sound: 'фюнф нах драй' }
          ]
        }
      ]
    }
  },
  {
    ...
    type: 'quiz_single',
    title: 'Тест на время',
    question: 'Как сказать "половина четвёртого" (3:30)?',
    options: ['drei Uhr dreißig', 'halb vier', 'Viertel nach drei', 'vier Uhr'],
    correctAnswer: 1,
    explanation: 'halb vier = половина четвёртого (до 4 осталось 30 минут)'
  },
  {
    id: 3,
    type: 'quiz_single',
    title: 'Тест на время',
    question: 'Как сказать "четверть четвёртого" (3:15)?',
    options: ['halb vier', 'Viertel nach drei', 'drei Uhr fünfzehn', 'Viertel vor vier'],
    correctAnswer: 1,
    explanation: 'Viertel nach drei = четверть после трёх'
  },
  {
    id: 4,
    type: 'match',
    title: 'Соответствия',
    question: 'Сопоставьте время:',
    pairs: [
      { left: 'halb sechs', right: '5:30' },
      { left: 'Viertel nach sieben', right: '7:15' },
      { left: 'zehn nach acht', right: '8:10' },
      { left: 'fünf vor zwölf', right: '11:55' }
    ]
  },
  {
    id: 5,
    type: 'quiz_multiple',
    title: 'Выберите способы сказать 3:45',
    question: 'Какие из этих способов означают 3:45?',
    options: ['halb vier', 'Viertel vor vier', 'drei Uhr fünf und vierzig', 'fünfzehn vor vier'],
    correctAnswer: [1, 2]
  },
  {
    id: 6,
...
export default function UhrzeitLesson() {
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <div className="text-blue-400 font-medium">Модуль 2 - Урок 3</div>
              <h1 className="text-3xl font-bold text-white mb-2">Uhrzeit</h1>
              <p className="text-white/80">Время</p>
            </div>
          </div>
          ...
          <CardContent className="space-y-6">
                  ...
                  <div key={sectionIndex} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                      {section.examples.map((item: any, itemIndex: number) => (
                        <div key={itemIndex} className="flex flex-col items-center p-3 bg-slate-800/50 rounded-xl border border-slate-700/50">
                          {item.number && <span className="text-lg font-bold text-white mb-1">{item.number}</span>}
                          <span className="text-sm font-semibold text-blue-400">{item.word}</span>
                          <span className="text-xs text-white/60">{item.sound}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {slide.type === 'completion' && (
              <div className="text-center space-y-6">
                ...
                <p className="text-xl text-white/80">You've learned telling time in German!</p>
            ...
            {slide.type === 'match' && (
              <div className="space-y-6">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-3">
                    <h4 className="font-medium text-blue-400">Time</h4>
                    {slide.pairs?.map((pair, index) => (
