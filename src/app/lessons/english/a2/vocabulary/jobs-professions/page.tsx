...
const slides: Slide[] = [
      ...
      sections: [
        {
          ...
          examples: [
            ...
            { word: 'pharmacist', translation: 'фармацевт' }
          ]
        },
        {
          subtitle: 'Education',
          examples: [
            ...
            { word: 'librarian', translation: 'библиотекарь' }
          ]
        },
        {
          subtitle: 'Business',
          examples: [
            ...
            { word: 'secretary', translation: 'секретарь' }
          ]
        }
      ]
    }
      ...
      sections: [
        ...
        },
        {
          ...
          examples: [
            ...
            { word: 'musician', translation: 'музыкант' },
  ...
  },
  {
    ...
    type: 'quiz_single',
    title: 'Word Meaning',
    question: '"A manager" is someone who...',
    options: ['teaches students', 'manages a team', 'cooks food', 'builds houses'],
    correctAnswer: 1
  },
  {
    id: 5,
    ...
    question: 'Match English jobs with Russian translations:',
    pairs: [
      { left: 'teacher', right: 'учитель' },
      { left: 'chef', right: 'шеф-повар' },
      { left: 'doctor', right: 'врач' },
      { left: 'artist', right: 'художник' }
    ]
  },
  {
    id: 6,
    ...
    options: ['professor', 'accountant', 'musician', 'driver'],
    correctAnswer: 0
  },
  {
    id: 7,
    type: 'quiz_single',
    title: 'Service Jobs',
    question: 'Who brings food in a restaurant?',
    options: ['chef', 'waiter', 'manager', 'librarian'],
    correctAnswer: 1
  },
  {
    id: 8,
...
export default function JobsProfessionsLesson() {
  ...
  useEffect(() => { window.dispatchEvent(new CustomEvent('lessonProgressUpdate', { detail: { lessonId: 'a2-6', progress, completed: currentSlide === slides.length - 1 } })) }, [currentSlide, progress])
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-emerald-900 to-slate-900 text-white">
      <header className="bg-slate-800/50 backdrop-blur-sm border-b border-slate-700/50"><div className="container mx-auto px-4 py-4"><div className="flex items-center justify-between"><Link href="/lessons/english/a2" className="flex items-center space-x-2"><div className="w-10 h-10 bg-emerald-600 rounded-xl flex items-center justify-center"><BookOpen className="w-6 h-6 text-white" /></div><span className="text-2xl font-bold">LinguaFlow</span></Link><div className="flex items-center space-x-4"><ThemeToggle /><Link href="/lessons/english/a2/vocabulary"><Button variant="outline" className="border-slate-600 hover:bg-slate-700"><ArrowLeft className="w-4 h-4 mr-2" />Back</Button></Link></div></div></div></header>
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto mb-8"><div className="flex items-center gap-4 mb-6"><div className="w-16 h-16 bg-emerald-600 rounded-2xl flex items-center justify-center"><BookOpen className="w-8 h-8 text-white" /></div><div><div className="text-sm text-white/60 mb-1">Урок 6 • Vocabulary</div><h1 className="text-3xl font-bold">Jobs & Professions</h1><p className="text-white/80">Профессии</p></div></div><div className="mb-6"><div className="flex items-center justify-between mb-2"><span className="text-sm text-white/60">Progress</span><span className="text-2xl font-bold text-emerald-400">{progress}%</span></div><div className="w-full bg-slate-800 rounded-full h-3"><div className="bg-gradient-to-r from-emerald-500 to-emerald-600 h-3 rounded-full transition-all duration-300" style={{ width: `${progress}%` }}></div></div></div><div className="flex items-center gap-6 text-sm text-white/60"><div className="flex items-center gap-2"><Clock className="w-4 h-4" /><span>18 min</span></div><div className="flex items-center gap-2"><Star className="w-4 h-4 text-yellow-400" /><span>45 XP</span></div></div></div>
        <Card className="max-w-4xl mx-auto bg-slate-800/30 backdrop-blur-sm border border-slate-700/50"><CardContent className="p-8">{renderSlideContent()}<div className="flex items-center justify-between mt-8 pt-6 border-t border-slate-700/50"><Button variant="outline" onClick={prevSlide} disabled={currentSlide === 0} className="border-slate-600 hover:bg-slate-700 text-white"><ArrowLeft className="w-4 h-4 mr-2" />Back</Button>{currentSlide === slides.length - 1 ? (<Link href="/lessons/english/a2/vocabulary"><Button className="bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white px-8"><Play className="w-4 h-4 mr-2" />Continue</Button></Link>) : (<Button onClick={nextSlide} disabled={slide.type !== 'theory' && !showFeedback[slide.id]} className="bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white">Continue<Play className="w-4 h-4 ml-2" /></Button>)}</div></CardContent></Card>
      </main>
    </div>
  )
}
