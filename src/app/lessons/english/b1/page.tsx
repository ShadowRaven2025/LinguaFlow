...
import { BookOpen, Brain, Trophy, Clock, Star, ArrowLeft, Search, CheckCircle, Lock, Play, GraduationCap, Sparkles, Target, MessageCircle, Users, Briefcase, Heart } from 'lucide-react'
...
const mockLessons = [
  ...
  },
  {
    ...
    description: 'Социальные отношения',
    duration: 18,
    xpReward: 50,
    category: 'vocabulary'
  },
  ...
  },
  {
    id: 'conv-1',
    title: 'Job Interview',
    description: 'Собеседование при приеме на работу',
    duration: 20,
    xpReward: 55,
  ...
  },
  {
    id: 'conv-2',
    title: 'Making Plans',
    description: 'Планирование и договоренности',
    duration: 18,
    xpReward: 50,
    category: 'conversation'
  },
  {
    ...
    description: 'Выражение эмоций и чувств',
    duration: 18,
    xpReward: 50,
    category: 'conversation'
  },
...
export default function EnglishB1Page() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('linguaflow-b1-progress')
    if (saved) {
      try {
  ...
  const getCorrectLessonPath = (lessonId: string, category: string) => {
    if (category === 'grammar') {
      return '/lessons/english/b1/grammar'
    }
    if (category === 'vocabulary') {
      return '/lessons/english/b1/vocabulary'
    }
    return `/lessons/english/b1`
  }
          ...
          <nav className="flex items-center justify-between">
            <Link href="/" className="flex items-center gap-3 group">
              ...
              <div>
                ...
                <div className="text-xs text-muted-foreground">Английский B1</div>
              </div>
            </Link>
            ...
            <CardContent className="p-6">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <ul className="space-y-2 text-sm text-muted-foreground">
                  <li className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500" /> Present Perfect и Past Perfect</li>
                  <li className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500" /> Условные предложения</li>
                  <li className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500" /> Модальные глаголы</li>
                  <li className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500" /> passive voice</li>
                </ul>
                <ul className="space-y-2 text-sm text-muted-foreground">
                  <li className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500" /> Профессиональная лексика</li>
                  <li className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500" /> Разговорные фразы</li>
                  <li className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500" /> Описание событий</li>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <Link href="/lessons/english-file">
              <Card className="group border-border/50 hover:border-primary/30 transition-all duration-300 card-hover opacity-0 animate-in-up" style={{ animationDelay: '550ms', animationFillMode: 'forwards' }}>
                ...
                <CardContent>
                  <div className="flex items-center gap-2">
                    <Badge className="bg-blue-500/10 text-blue-500">B1</Badge>
            ...
            <Link href="/lessons/english-grammar-in-use">
              <Card className="group border-border/50 hover:border-primary/30 transition-all duration-300 card-hover opacity-0 animate-in-up" style={{ animationDelay: '600ms', animationFillMode: 'forwards' }}>
                ...
                <CardContent>
                  <div className="flex items-center gap-2">
                    <Badge className="bg-purple-500/10 text-purple-500">B1-B2</Badge>
            ...
            <Link href="/lessons/english-file/intermediate">
                ...
                <CardHeader className="pb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <CardDescription className="text-bottom">English File</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="flex items-center gap-2">
                    <Badge className="bg-green-500/10 text-green-500">B1</Badge>
