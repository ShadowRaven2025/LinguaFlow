...
import { BookOpen, Brain, Trophy, Clock, Star, ArrowLeft, Search, CheckCircle, Lock, Play, GraduationCap, Sparkles, Target, MessageCircle, UtensilsCrossed, ShoppingBag, MapPin } from 'lucide-react'

const mockLessons = [
  {
    id: 'grammar-1',
    title: 'Präsens',
    description: 'Настоящее время глаголов',
    duration: 18,
    xpReward: 50,
    category: 'grammar'
  },
  ...
  },
  {
    id: 'grammar-3',
    title: 'Perfekt',
    description: 'Перфект - прошедшее время',
    duration: 22,
    xpReward: 60,
    category: 'grammar'
  },
  ...
  },
  {
    id: 'vocab-3',
    title: 'Richtungen',
    description: 'Направления и места',
    duration: 15,
    xpReward: 40,
    category: 'vocabulary'
  },
  ...
  },
  {
    id: 'vocab-5',
    title: 'Urlaub',
    description: 'Отпуск и путешествия',
    duration: 18,
    xpReward: 50,
    category: 'vocabulary'
  },
  {
    ...
    description: 'Назначение встречи',
    duration: 15,
    xpReward: 45,
...
export default function GermanA2Page() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('linguaflow-german-a2-progress')
    if (saved) {
      try {
  ...
  const getCorrectLessonPath = (lessonId: string, category: string) => {
    if (category === 'grammar') {
      return '/lessons/german/a2/grammar'
    }
    if (category === 'vocabulary') {
      return '/lessons/german/a2/vocabulary'
    }
    if (category === 'conversation') {
      return '/lessons/german/a2/conversation'
    }
    return `/lessons/german/a2`
  }
          ...
          <nav className="flex items-center justify-between">
            <Link href="/" className="flex items-center gap-3 group">
              ...
              <div>
                ...
                <div className="text-xs text-muted-foreground">Немецкий A2</div>
              </div>
            </Link>
        ...
        <div className="animate-in-up opacity-0 mb-12 max-w-3xl mx-auto" style={{ animationDelay: '300ms', animationFillMode: 'forwards' }}>
          <Card className="border-border/50">
            <CardContent className="p-6">
              <h3 className="text-lg font-bold text-foreground mb-4">Что вы изучите на уровне A2:</h3>
              <div className="grid md:grid-cols-2 gap-4">
                <ul className="space-y-2 text-sm text-muted-foreground">
                  <li className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500" /> Прошедшее время Präteritum</li>
                  <li className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500" /> Перфект Perfekt</li>
                  <li className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500" /> Винительный падеж Akkusativ</li>
                  <li className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500" /> Дательный падеж Dativ</li>
                </ul>
                <ul className="space-y-2 text-sm text-muted-foreground">
                  <li className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500" /> Лексика ресторана</li>
                  <li className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500" /> Покупки и магазины</li>
                  <li className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500" /> Путешествия</li>
              ...
              <Card className="group border-border/50 hover:border-primary/30 transition-all duration-300 card-hover opacity-0 animate-in-up" style={{ animationDelay: '600ms', animationFillMode: 'forwards' }}>
                <CardHeader className="pb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle className="text-base">Menschen</CardTitle>
                ...
                <CardContent>
                  <div className="flex items-center gap-2">
                    <Badge className="bg-green-500/10 text-green-500">A1-B1</Badge>
                    <span className="text-xs text-muted-foreground">24 урока</span>
                  </div>
                </CardContent>
              </Card>
            </Link>
                ...
                <CardHeader className="pb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <CardDescription className="text-sm">B1-B2</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
